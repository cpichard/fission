...
template <typename NT> inline size_t NbInputs() { return NT::s_nbInputs; }

template <typename NT> inline size_t NbParameters() {
    return NT::s_nbParameters;
}

inline size_t NbInputs(const NodeDesc *node) { return node->nbInputs(); }

inline size_t NbParameters(const NodeDesc *node) {
    return node->nbParameters();
}

template <typename NT>
inline const NodeDesc::Input *Inputs(const NT *nodetype) {
    return nodetype->inputs();
}

template <typename NT>
inline const NodeDesc::Output *Output(const NT *nodetype) {
    return nodetype->output();
}

template <typename NT>
inline const NodeDesc::Param *Parameters(const NT *nodetype) {
    return nodetype->parameters();
}
}; // namespace fission

// ============== Macros to make a new node from the api side ==================
    ...
    class NodeName : public NodeDesc {                                         \
      ...
      public:                                                                  \
        ...
        inline size_t nbInputs() const {                                       \
            return NodeName::s_nbInputs;                                       \
        };                                                                     \
        inline size_t nbParameters() const {                                   \
            return NodeName::s_nbParams;                                       \
        };                                                                     \
...
#define NewParam(val, typ)                                                     \
    NodeDesc::Param(val, Type<fission::typ>(), "standard")
